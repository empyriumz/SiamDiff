...
class RES(tasks.Task, core.Configurable):
    ...
    def apply_mask(self, graph):
        residue_mask = scatter_sum(
            graph.ca_mask.float(), graph.atom2residue, dim=0, dim_size=graph.num_residue
        ).bool()
    ...
    def predict_and_target(self, batch, all_loss=None, metric=None):
        ...
        output_feature = (
            output["node_feature"]
            if graph.view in ["node", "atom"]
            else output.get("residue_feature", output.get("node_feature"))
        )
...
class MSP(tasks.InteractionPrediction):
    ...
    def predict(self, batch, all_loss=None, metric=None):
        ...
        output1 = scatter_add(
            output1["node_feature"] * node_mask1,
            graph1.atom2graph,
            dim=0,
            dim_size=graph1.batch_size,
        ) / (
            scatter_add(
                node_mask1, graph1.atom2graph, dim=0, dim_size=graph1.batch_size
            )
            + 1e-10
        )
        node_mask2 = residue_mask[graph2.atom2residue].float().unsqueeze(-1)
        output2 = scatter_add(
            output2["node_feature"] * node_mask2,
            graph2.atom2graph,
            dim=0,
            dim_size=graph2.batch_size,
        ) / (
            scatter_add(
                node_mask2, graph2.atom2graph, dim=0, dim_size=graph2.batch_size
            )
            + 1e-10
        )
        pred = self.mlp(torch.cat([output1, output2], dim=-1))
...
class SiamDiff(tasks.Task, core.Configurable):
    ...
    def add_seq_noise(self, graph, noise_level):
        ...
        mask_rate = (self.max_mask_ratio - self.min_mask_ratio) * (
            (noise_level + 1) / self.num_noise_level
        ) + self.min_mask_ratio
        ...
        selected_residue = torch.zeros(
            (graph.num_residue,), dtype=torch.bool, device=graph.device
        )
        selected_residue[node_index] = 1
        # only keep backbone atoms of the selected residues
        node_mask = (
            (graph.atom_name == graph.atom_name2id["CA"])
    ...
    def add_struct_noise(self, graph, noise_level):
        ...
        dist = (graph.node_position[node_in] - graph.node_position[node_out]).norm(
            dim=-1
        )
        perturb_noise = torch.randn_like(graph.node_position)
        graph.node_position = graph.node_position + perturb_noise * (
            (1.0 - a_pos).sqrt() / a_pos.sqrt()
        ).unsqueeze(-1)
        perturbed_dist = (
        ...
        if (
            ...
            and self.graph_construction_model.edge_feature == "gearnet"
        ):
    ...
    def eq_transform(self, score_d, graph):
        ...
        score_pos = scatter_mean(
            dd_dr * score_d.unsqueeze(-1), node_in, dim=0, dim_size=graph.num_node
        ) + scatter_mean(
            -dd_dr * score_d.unsqueeze(-1), node_out, dim=0, dim_size=graph.num_node
        )
        return score_pos
    ...
    def predict_and_target(self, batch, all_loss=None, metric=None):
        ...
        # add structure noise
        if self.gamma > 0.0:
            graph1, perturbed_dist1, struct_target1 = self.add_struct_noise(
                graph1, noise_level
            )
            if self.use_MI:
                graph2, perturbed_dist2, struct_target2 = self.add_struct_noise(
                    graph2, noise_level
                )

        output1 = self.model(graph1, graph1.node_feature.float(), all_loss, metric)[
            "node_feature"
        ]
        if self.use_MI:
            output2 = self.model(graph2, graph2.node_feature.float(), all_loss, metric)[
                "node_feature"
            ]
        else:
            output2 = output1
        ...
        if self.gamma > 0.0:
            ...
            if self.use_MI:
                ...
                loss2 = scatter_mean(
                    loss2, graph2.node2graph, dim=0, dim_size=graph2.batch_size
                ).mean()
            else:
                loss2 = loss1
        ...
        if self.gamma < 1.0:
            ...
            loss1 = scatter_mean(
                loss1,
                graph1.residue2graph[node_index],
                dim=0,
                dim_size=graph1.batch_size,
            ).mean()
            ...
            if self.use_MI:
                ...
                loss2 = scatter_mean(
                    loss2,
                    graph2.residue2graph[node_index],
                    dim=0,
                    dim_size=graph2.batch_size,
                ).mean()
