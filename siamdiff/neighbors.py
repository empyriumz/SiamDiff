...
def remove_unmatching(neighbors, df0, df1):
    ...
    _, res_to_idx = _get_idx_to_res_mapping(pd.concat([df0, df1]))
    to_drop = []
    for i, neighbor in neighbors.iterrows():
        res0 = tuple(
            neighbor[
                ["ensemble0", "subunit0", "structure0", "model0", "chain0", "residue0"]
            ]
        )
        res1 = tuple(
            neighbor[
                ["ensemble1", "subunit1", "structure1", "model1", "chain1", "residue1"]
            ]
        )
